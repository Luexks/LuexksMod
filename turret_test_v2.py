...
def calculate_vertices(angle_deg, leg_length):
    ...
    return [(x1, y1), (x2, y2), (x3, y3)], height
...
def calculate_radius(vertices, height):
    """Calculate the radius of the circle centered at the triangle's centroid based on height."""
    ...
    # Adjust diameter based on height (e.g., make it 1.5 times the height)
    diameter = 1.5 * height
    
    return centroid, diameter / 2
...
def update_plot(angle_deg):
    ...
    vertices, height = calculate_vertices(angle_deg, leg_length)
    centroid, radius = calculate_radius(vertices, height)
