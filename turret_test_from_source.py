...
def calculate_radius(vertices, centroid):
    """Calculate the radius based on the given algorithm."""
    minradius = math.dist(centroid, vertices[0])
    maxradius = minradius

    for i in range(len(vertices)):
        distance_to_vertex = math.dist(centroid, vertices[i])
        maxradius = max(maxradius, distance_to_vertex)

        # Calculate the minimum radius between two vertices
        next_index = (i + 1) % len(vertices)
        midpoint = lerp(vertices[i], vertices[next_index], 0.5)
        minradius = min(minradius, math.dist(centroid, midpoint))

    # The radius is the minimum between maxradius and minradius
    return min(maxradius, minradius)
